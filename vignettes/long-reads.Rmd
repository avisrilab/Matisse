...
targets the `"transcript"` assay and then runs PCA -- useful because isoform
variation is subtler than gene-expression variation and benefits from a wider
PCA space.
...
> the row names in `GetPSI(obj)`, so use the same string when calling
> `PlotUMAP()` or subsetting.
