...
- `nFeature_isoform` -- number of transcripts with at least one read per cell

---

## Step 2 -- Calculate PSI

Call `CalculatePSI()` explicitly to compute Percent Spliced In (PSI) for
each splice event in each cell. For each cell and event, Matisse sums
transcript counts from inclusion isoforms and exclusion isoforms, then
computes:
