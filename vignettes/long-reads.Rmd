...
variability. `SCTransform()` on a Matisse object in event mode automatically
targets the `"transcript"` assay. Run `RunPCA()` explicitly afterwards.
...
`CoveragePlot()` draws junction arcs scaled by aggregate read count, coloured
...
> `PlotUMAP()`, `CoveragePlot()`, or subsetting.
